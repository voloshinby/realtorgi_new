...
class LotController extends BaseController
{
    ...
    {
        $lots = $this->lot->latest()->with(['auction_files', 'auction_gallery', 'auction', 'gallery', 'files', 'category'])->withCount('users')->paginate(1000);
    ...
    {
        $lot = $this->lot->with(['auction_files', 'auction_gallery', 'auction', 'gallery', 'files', 'category'])->withCount('users')->findOrFail($id);
    ...
    public function userList($id){
        ...
        $userLots = AuctionConfirm::latest()->where('user_id', $id)->where('confirmed_admin', 1)->with('gallery', 'lot', 'auction')->where('confirmed_user', 1)->get();
    ...
    public function categoryFilter(Request $request){
        ...
        $lots = $this->lot->where('category_id', $category->id)->with(['auction', 'gallery', 'files', 'category'])->where('status', '<>', 'На модерации')->paginate(1000);
