    ...
    <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
        ...
        <li class="nav-item">
            <router-link to="/admin/auctionConfirms" class="nav-link">
                ...
                <p>
                    Заявки на участие
                    @if(DB::table('auction_confirms')->where('confirmed_admin' , false)->count() > 0)
                        <span style="color: orange;">
                        ({{ DB::table('auction_confirms')->where('confirmed_admin', false)->count() }})
                    </span>
                    @endif
                </p>
            </router-link>
        ...
        @can('isAdmin')
            ...
            <li class="nav-item">
                <router-link to="/admin/change_users" class="nav-link">
                    <i class="fa fa-users nav-icon blue"></i>
                    <p>Изменения данных
                        @if(DB::table('user_notifications')->where('confirmed', 0)->count() > 0)
                            <span style="color: orange;">
                        ({{ DB::table('user_notifications')->where('confirmed', 0)->count() }})
                    </span>
                        @endif
                    </p>
                </router-link>
