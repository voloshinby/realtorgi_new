    ...
    <div class="customer-request-wrapper">
      ...
      <div v-if="lots.length > 0" class="requests-table">
        <div class="requests-table-wrapper">
          ...
          <div class="table-row" v-for="( lot, index ) in lots" :key="index">
            ...
            <nuxt-link v-if="lot.lot" :to="{name: 'auctions-id', params: { id: lot.lot.id }}" class="name">
              {{ lot.lot.name }}
            </nuxt-link>
            ...
            <span
              v-if="lot.lot && (lot.confirmed_admin === 0 && (lot.lot.status === 'Предстоящие' || lot.lot.status === 'Повторные'))"
              class="confirm-status">Ожидает подтверждения администратором</span>
            <span @click="showParticipationPopup(lot)"
                  v-else-if="lot.lot && (lot.confirmed_admin === 1 && lot.confirmed_user === 0 && (lot.lot.status === 'Предстоящие' || lot.lot.status === 'Повторные'))"
      ...
      <div class="find-more">
        <nuxt-link :to="{ name: 'auctions' }">
          <button class="find-more-button">Найти еще аукционы
            <arrow-right-icon/>
          </button>
        </nuxt-link>
      </div>
...
export default {
  ...
  data: () => ({
    ...
    canApproveParticipationAuction: false,
    diffTimeBeforeApprove: null,
    labelTime: 'ч.',
  }),
  components: {
    ArrowRightIcon,
  ...
  methods: {
    showParticipationPopup(lot) {
      this.choosenLot = lot;
      let timestamp = this.choosenLot.auction.start_selling;
      let currentDate = new Date();
      let auctionStartDateTime = new Date(timestamp * 1000);
    ...
    deleteRequest() {
      ...
      this.$notify({
        'group': 'user-notifications',
        'title': `<div class='title'>Заявка на удаление отправлена</div>`,
        'text': ``,
        'duration': 5000
      })
    },
