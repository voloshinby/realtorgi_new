    ...
    <div v-if="openParticipationPopup" class="participation-popup" @click="openParticipationPopup = !openParticipationPopup">
      <div class="participation-popup-form" @click.stop>
        <div class="participation-popup-form-wrapper">
          ...
          <div class="subtitle">Подтверждаете ли Вы свое участие в торгах по данному лоту ({{ choosenAuction.name }})?</div>
    ...
    <div v-if="this.$route.name === 'auctions'" class="filters">
       ...
       <div class="filters-list">
          ...
          <div class="filter-list-wrapper">
            <div @click="chooseFilter({name: 'Все'})" :class="{ active: (choosenFilter.name === 'Все') }" class="filter"><span class="filter-name">Все </span></div>
            <div v-for="(category, index) in categories" :key="index" @click="chooseFilter(category)" :class="{ active: (category.name === choosenFilter.name) }" class="filter"><span class="filter-name">{{ category.name }} <span class="lots-nubmer">({{ category.lot_count }})</span></span></div>
          </div>
          <h2 class="filters-title-mobile" @click="showFilters = !showFilters"><chevron-down-icon :class="{ opened: (showFilters) }"/>Категории</h2>
          <div class="filter-list-wrapper-mobile" :class="{hidden: (!showFilters)}">
            <div @click="chooseFilter({name: 'Все'})" :class="{ active: (choosenFilter.name === 'Все') }" class="filter"><span class="filter-name">Все </span></div>
            <div v-for="(category, index) in categories" :key="index" @click="chooseFilter(category)" :class="{ active: (category.name === choosenFilter.name) }" class="filter"><span class="filter-name">{{ category.name }} <span class="lots-nubmer">({{ category.lot_count }})</span></span></div>
          </div>
        <div class="filters-searches-mobile">
          <h2 class="filters-searches-title-mobile" @click="showSearches = !showSearches"><chevron-down-icon :class="{ opened: (showSearches) }"/>Поиск</h2>
        ...
        <div class="auction-sorting-mobile" v-if="auctions.length > 0">
          <h2 class="auction-sorting-title-mobile"  @click="showSorting = !showSorting"><chevron-down-icon :class="{ opened: (showSorting) }"/>Сортировка</h2>
        ...
        <div class="auction-sorting-mobile cities" v-if="auctions.length > 0">
          <h2 class="auction-sorting-title-mobile"  @click="showCities = !showCities"><chevron-down-icon :class="{ opened: (showCities) }"/>Область</h2>
    ...
    <div class="auctions">
      ...
      <div class="auction-filters" v-if="this.$route.name === 'auctions'">
        <div class="status-filters">
          <button v-for="(status, index) in statuses" :key="index" @click="chooseStatus(status)" class="status-filter" :class="{'status-filter-active': (status.name === choosenStatuses.name)}">{{ status.name }}</button>
        </div>
      ...
      <div v-if="(auctions.length > 0) && ((this.$route.name === 'auctions') || (this.$route.name === 'auctions-all-id')) && (paginatedData.length !== 0)">
...
export default {
  ...
  async fetch() {
    if (this.$route.name !== 'auctions-all-id') {
      if (this.$route.query.category && this.$route.query.category !== 'Все') {
        const data = await this.$axios.post('https://realtorgi.by/admin/api/admin/categoryFilter', {
      ...
      await this.$axios.get('https://realtorgi.by/admin/api/admin/category').then((response) => {
      ...
      await this.$axios.get('https://realtorgi.by/admin/api/admin/country').then((response) => {
        this.cities = response.data.data.data
      })
    }
    else {
      const data = await this.$axios.get(`https://realtorgi.by/admin/api/admin/auction/lots/${this.$route.params.id}`)
      this.auctions = data.data.data.data
      this.auctions = this.auctions.filter(item => !item.status.includes("Черновик") && !item.status.includes("На модерации"));
      this.filteredByStatusAuctions = this.auctions
      this.pageCount
  ...
  methods:{
    ...
    submitParticipation() {
      this.$axios.put(`https://realtorgi.by/admin/api/admin/auctionConfirm/${this.choosenRequest.id}`, {
    ...
    chooseFilter(category) {
      ...
      if (category.name !== 'Все') {
        this.$axios.post('https://realtorgi.by/admin/api/admin/categoryFilter', {
